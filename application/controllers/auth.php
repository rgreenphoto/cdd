...
class Auth extends Main_Controller {
	...
	function login()
	{
		$this->data['title'] = "Login";
                $this->data['attributes'] = array('class' => 'form-signin');
                $this->data['hidden'] = array('previous_page' => !empty($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER']: '');
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				if(!empty($_POST['previous_page']) && $_POST['previous_page'] !== base_url().'auth/login') {
                                    redirect(''.$_POST['previous_page'].'', 'refresh');
                                } else {
		...
		else
		{
			...
			//set the flash data error message if there is one
			//$this->data['message'] = $this->ion_auth->errors();//(validation_errors()) ? validation_errors() : $this->session->flashdata('message');
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false) {
                    ...
                    if($identity) {
                        ...
                        //set any errors and display the form
                        $this->data['message'] = 'Email address not found. Please contact <a href="mailto:webmaster@coloradodiscdogs.com">webmaster@coloradodiscdogs.com</a> for assistance.';
                        $this->data['main'] = 'auth/forgot_password';
                        $this->load->view('login_layout', $this->data);
			...
			if ($this->form_validation->run() == false)
			{
				...
				$this->data['new_password'] = array(
					...
					'pattern' => '^.{'.$this->data['min_password_length'].'}.*$',
                                        'class' => 'input-block-level',
                                        'rel' => 'tooltip',
                                        'title' => 'Password should contain a mix of alpha-numeric, uppercase and special characters(!@#_*)'
				);
				$this->data['new_password_confirm'] = array(
					'name' => 'new_confirm',
					'id'   => 'new_confirm',
...
//		}
		...
		if ($this->form_validation->run() == true)
		{
			...
			$password = $this->input->post('password');
                        $slug = strtolower(url_title($this->input->post('first_name').' '.$this->input->post('last_name')));
		...
		{
			//try to log in the new user if not redirect to login form
                        $remember = '';
