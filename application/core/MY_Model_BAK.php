...
class MY_Model extends CI_Model {
    ...
    public function set_post_options($array) {
        //basic way to set $_POST into model array for add/update
        $options = array();
        if(!empty($array)) {
	...
	function add($options = array()) {
		...
		$default = array(
			'created' => date($this->config->item('log_date_format')),
                        'created_by' => $this->data['the_user']->id,
			'modified' => date($this->config->item('log_date_format')),
                        'modified_by' => $this->data['the_user']->id
		);
	...
	function _default($defaults, $options)
	{
       ...
       function _array_to_select($results, $value = 'id', $key = 'title', $add_blank = '') {
